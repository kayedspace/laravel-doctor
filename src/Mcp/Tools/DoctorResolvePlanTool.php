...
class DoctorResolvePlanTool extends Tool
{
    protected string $name = 'doctor_resolve_plan';

    protected string $title = 'Laravel Doctor Resolve Plan';

    protected string $description = 'Group findings into a read-only remediation plan.';
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            'scopePreset' => $schema->string()
                ...
                ->description('Scan scope to plan from. Default "full"; "manual" scans the given paths; "changed" scans git-changed files; "laravel" scans the app/config/database/routes/views tree.'),
    ...
    public function handle(Request $request): mixed
    {
        ...
        $report = McpReadOnlyScanner::scan($this->manager, $doctorRequest);

        $result = $this->planner->plan($report->getFindings()) + [
            'status' => $report->getStatus(),
            'errors' => array_map(static fn ($error): array => $error->toArray(), $report->getErrors()),
        ];
